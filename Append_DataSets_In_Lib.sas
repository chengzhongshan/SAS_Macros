*options mprint;
%macro Append_DataSets_In_Lib(lib,excel,excluded,add_dsd_name,dsd_not_contain_rgx,dsd_contain_rgx,dsdout);
...
%end;
%do i=1 %to &n;
if %str(xyz&i) then %str(&add_dsd_name)=%str("%sysfunc(compbl(&&name&i))");
%end;;
run;
...
%Append_DataSets_In_Lib(
lib=pooled,
excel=1,
excluded=,
add_dsd_name=dsd_name,
dsd_not_contain_rgx=FilterDatabase tagged Meta,
dsd_contain_rgx=,
dsdout=DC_Pooled_Analysis);
...
excluded=,
add_dsd_name=dsd_name,
dsd_not_contain_rgx=FilterDatabase tagged Baseline LPS FLU IFNb,
dsd_contain_rgx=,
dsdout=DC_Pooled_Analysis_Meta);
